...
class SessionCipher {
    ...
    async encrypt(data) {
        ...
        return await this.queueJob(async () => {
            ...
            if (!record) {
                console.warn(`[encrypt] No se encontró ningún registro de sesión para: ${this.addr?.id}`);
                throw new errors.SessionError("No sessions");
            }

            const session = record.getOpenSession();
            if (!session) {
                console.warn(`[encrypt] No hay sesión abierta disponible para: ${this.addr?.id}`);
            ...
            if (!chain) {
                throw new Error("No se encontró una cadena válida en la sesión");
            }
